...
public class ColorPicker : MonoBehaviour
{
    ...
    public Slider hComponent;
    public Slider sComponent;
    public Slider vComponent;
    public Slider aComponent;
    public InputField hexaComponent;
    public RawImage colorComponent;

    [Header("position on create")]
    ...
    {   
        ...
        if(done)
        {
            ...
            instance.GetComponent<RectTransform>().anchoredPosition = new Vector2(instance.anchoredPosX, instance.anchoredPosY);
    ...
    private void RecalculateMenu(bool recalculateHSV)
    {
        ...
        //GBO : add HSV sliders
        hComponent.value = (float)modifiedHsv.H;
        hComponent.transform.GetChild(3).GetComponent<InputField>().text = modifiedHsv.H.ToString();
        sComponent.value = (float)modifiedHsv.S;
        sComponent.transform.GetChild(3).GetComponent<InputField>().text = (modifiedHsv.S*100.0f).ToString();
        vComponent.value = (float)modifiedHsv.V;
        vComponent.transform.GetChild(3).GetComponent<InputField>().text = (modifiedHsv.V*100.0f).ToString();
        // end GBO
        if (useA)
        {
        ...
        //GBO : add HSV sliders
        sComponent.transform.GetChild(0).GetComponent<RawImage>().color = new HSV(modifiedHsv.H, 1, modifiedHsv.V).ToColor();
        sComponent.transform.GetChild(0).GetChild(0).GetComponent<RawImage>().color = new HSV(modifiedHsv.H, 0, modifiedHsv.V).ToColor();
        vComponent.transform.GetChild(0).GetComponent<RawImage>().color = new HSV(modifiedHsv.H, modifiedHsv.S, 1).ToColor();
        vComponent.transform.GetChild(0).GetChild(0).GetComponent<RawImage>().color = new HSV(modifiedHsv.H, modifiedHsv.S, 0).ToColor();
        // end GBO
    ...
    public void SetH(string value)
    {
        if (interact)
        {
            modifiedHsv.H = (double)Mathf.Clamp(int.Parse(value), 0, 360);
            RecalculateMenu(false);
        }
    }
    //gets a Slider value
    ...
    public void SetS(string value)
    {
        if (interact)
        {
            modifiedHsv.S = (double)Mathf.Clamp(int.Parse(value), 0, 100) / 100d;
            RecalculateMenu(false);
        }
    }
    //gets a Slider value
    ...
    public void SetV(string value)
    {
        if (interact)
        {
            modifiedHsv.V = (double)Mathf.Clamp(int.Parse(value), 0, 100) / 100d;
            RecalculateMenu(false);
        }
    }
